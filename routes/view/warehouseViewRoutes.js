...
// Add new warehouse route to render form
router.get('/warehouses/new', async (req, res) => {
  try {
    const dealers = await Dealer.find({});
    res.render('warehouseForm', { warehouse: null, dealers });
  } catch (error) {
    console.error(`Error displaying new warehouse form: ${error.message}`, error.stack);
    res.status(500).send('Error fetching dealers for new warehouse form');
  }
});
...
router.get('/warehouses/:id', async (req, res) => {
  try {
    ...
    const dealers = await Dealer.find({});
    res.render('warehouseForm', { warehouse, dealers });
  } catch (error) {
